...
func dispatch_operation(op: String, params: Dictionary) -> Dictionary:
    match op:
        ...
        "import_asset":
            return import_asset(params)
        "get_performance_metrics":
            return get_performance_metrics(params)
        "get_memory_breakdown":
            return get_memory_breakdown(params)
        "create_behavior_tree":
            return create_behavior_tree(params)
        "configure_blackboard":
            return configure_blackboard(params)
        _:
...
func replay_input_sequence(params: Dictionary) -> Dictionary:
    ...
    for i in range(sequence.size()):
        ...
        var sim_res = simulate_input(sim_step)
    ...
    if take_ss:
        var ss_res = take_screenshot(params)
...
func import_asset(params: Dictionary) -> Dictionary:
    ...
    if Engine.has_singleton("EditorInterface"):
        ...
        if ei and ei.has_method("get_resource_filesystem"):
            ...
            if efs and efs.has_method("reimport_files"):
...
func get_memory_breakdown(params: Dictionary = {}) -> Dictionary:
...
func create_behavior_tree(params: Dictionary) -> Dictionary:
    ...
    if not scene_root:
        ...
        if not ClassDB.class_exists(base_type):
            base_type = "Node"
        scene_root = ClassDB.instantiate(base_type) as Node
        ...
        if not parent_node:
            return {"status": "error", "error": "Parent node not found in scene: " + parent_path}

        var bt_type = root_type
        if not ClassDB.class_exists(bt_type):
            bt_type = "Node"
        root_node = ClassDB.instantiate(bt_type) as Node
    ...
    if typeof(nodes) == TYPE_ARRAY:
        for n_def in nodes:
            if typeof(n_def) == TYPE_DICTIONARY:
                var child_info = _build_bt_node_recursive(root_node, n_def, scene_root, tree_type)
...
func _build_bt_node_recursive(parent_node: Node, node_def: Dictionary, scene_root: Node, tree_type: String = "standard") -> Dictionary:
    ...
    if typeof(children_defs) == TYPE_ARRAY:
        for c_def in children_defs:
            if typeof(c_def) == TYPE_DICTIONARY:
                children_info.append(_build_bt_node_recursive(node, c_def, scene_root, tree_type))
...
func configure_blackboard(params: Dictionary) -> Dictionary:
    var scene_path = String(params.get("scene_path", ""))
